...
void ABlasterGameState::MulticastData_Implementation(const TArray<ABlasterPlayerState*>& Data)
{
	...
	UE_LOG(LogTemp, Error, TEXT("5. Inside multicast. BlasterPlayerStates.Num(): %d"), BlasterPlayerStates.Num());
}

void ABlasterGameState::GetAllPlayerStates_Implementation()
{
	UE_LOG(LogTemp, Error, TEXT("3. ServerInside GetAllPlayerStates, before for"));
	for (FConstPlayerControllerIterator It = GetWorld()->GetPlayerControllerIterator(); It; It++)
	{
		...
		if (PlayerController)
		{
			...
			if (BlasterPlayerState)
			{
				...
				UE_LOG(LogTemp, Error, TEXT("ServerPlayerState: %s"), *BlasterPlayerState->GetPlayerName());
				UE_LOG(LogTemp, Error, TEXT("ServerPlayerState.Num(): %d"), BlasterPlayerStates.Num());
			}
		}
	}
	UE_LOG(LogTemp, Error, TEXT("3.2 Inside GetAllPlayerStates, after for"));
	// Replicate the updated array to all clients
	if (HasAuthority())
	{
		UE_LOG(LogTemp, Error, TEXT("4. Authority check"));
		BlasterPlayerStates.Sort(); // ensure the array is sorted to ensure consistent order
		MulticastData(BlasterPlayerStates);
		UE_LOG(LogTemp, Error, TEXT("6. After multicast"));
	}
}
